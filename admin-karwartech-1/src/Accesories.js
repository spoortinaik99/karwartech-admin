...
const Accesories = () => {
    ...
    const fetchMobiles = async () => {
      try {
        const response = await axios.get('https://karwartech-backend.onrender.com/added-data-acc');
    ...
    const handleUpload = async (event) => {
      ...
      try {
        await axios.post('https://karwartech-backend.onrender.com/uploadacc', formData, {
          headers: {
    ...
    const handleClearData = async () => {
      try {
        await axios.delete('https://karwartech-backend.onrender.com/clear-data-acc');
        setMobiles([]);
      } catch (error) {
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Accesories Gallery</h1>
